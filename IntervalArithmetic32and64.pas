...
// z jaka dokladnoscia jest przedzial
...
// jezeli wprowadzimy liczbe rzeczywista, to zrobi nam przedzial
function left_read(const sa: string): Extended;
// odczytuje lewy koniec przedzialu
function right_read(const sa: string): Extended;
// odczytuje prawy koniec przedzialu
...
// Sprawdzenie czy w przedziale nie ma zera
function NotZero(x: interval): boolean;
...
{$ENDIF}
    ...
    function iabs(const x: interval): interval;
    // funkcja wartoœæ bezwzglêdna dla arytmetyki przedzialowej
    begin
      if (x.a <= 0) and (x.b <= 0) then
      begin
        Result.a := abs(x.b);
        Result.b := abs(x.a);
      end else if (x.a < 0) and (x.b > 0) then
      begin
        Result.a := 0;
        Result.b := max(abs(x.b), abs(x.a));
      end else begin
        Result.a := x.a;
        Result.b := x.b;
      end;
    ...
    function zmiennaNaPrzedzal(const x: Extended): interval;
    begin
      Result.a := x;
      Result.b := x;
    end;
